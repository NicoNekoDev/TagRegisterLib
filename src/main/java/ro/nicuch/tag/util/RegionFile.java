...
public class RegionFile implements AutoCloseable {
    ...
    public final Chunk getChunkCompoundTag(final int chunk_x, final int chunk_y, final int chunk_z) {
        ...
        Chunk chunk = new Chunk(); // default chunk
        ...
        try {
            ...
            try (FastByteArrayInputStream byteArrayInput = new FastByteArrayInputStream(buffer.array())) {
                try (StreamNetInput netInput = new StreamNetInput(byteArrayInput)) {
                    return Chunk.read(netInput);
                }
            } catch (Exception ex) {
                ex.printStackTrace();
            }
    ...
    public final void putChunkCompoundTag(final int chunk_x, final int chunk_y, final int chunk_z, final Chunk chunk) {
        int region_chunk_x = chunk_x & 31;
        int region_chunk_y = chunk_y & 31;
        int region_chunk_z = chunk_z & 31;
        if (this.outOfBounds(region_chunk_x, region_chunk_y, region_chunk_z)) {
        ...
        lock.lock();
        try {
            try (FastByteArrayOutputStream byteArrayOutput = new FastByteArrayOutputStream()) {
                try (StreamNetOutput netOutput = new StreamNetOutput(byteArrayOutput)) {
                    Chunk.write(netOutput, chunk);
                }
                byte[] array = byteArrayOutput.array;
    ...
    public final void setOffset(final int x, final int y, final int z, final int offset) {
        try {
            int location = x + (z * 32) + (y * 1024);
